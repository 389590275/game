 ...
 */
public class StandPlayerState implements IPlayerState {

    private Player player = CacheManager.INSTANCE.player;

    public StandPlayerState() {
        TextureRegion[] frames = ResPlayer.getStandTextureRegion(PlayerActor.PID,
                player.isFlip());
    ...
    public void keyDown(InputEvent event, int keycode) {
        switch (keycode) {
            ...
            case Input.Keys.RIGHT:
                player.setV(Constants.ROLE_V);
                player.setDirection(keycode);
                player.setState(new MovePlayerState());
                break;
            case Input.Keys.A:
                player.setState(new AtkPlayerState());
                break;
        }
    }
